...
"""zero Multi speaker version"""
class Zero_TextAudioSpeakerLoader(torch.utils.data.Dataset):
    """
        1) loads audio, speaker_id, text pairs
        2) normalizes text and converts them to sequences of integers
        3) computes spectrograms from audio files.
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        # separate filename, speaker_id and text
        try:
            audiopath, sid, text = audiopath_sid_text[0], audiopath_sid_text[1], audiopath_sid_text[3]
        except:
...
class Zero_TextAudioSpeakerCollate():
    """ Zero-pads model inputs and targets
    """
    def __init__(self, slice_length, return_ids=False):
    ...
    def __call__(self, batch):
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            if emb_mel.size(1) > self.slice_length +100:
                ...
                mel_emb_padded[i, :, :self.slice_length] = emb_mel[:, :self.slice_length]
            else :
                mel_emb_padded[i, :, :emb_mel.size(1)] = emb_mel
        ...
        if self.return_ids:
            return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, mel_emb_padded, ids_sorted_decreasing
        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, mel_emb_padded
